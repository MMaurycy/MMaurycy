<div align="center">

```
┌──(marcin㉿rzeszow)-[~/data-engineering]
└─$ whoami --verbose
```

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=26&duration=3000&pause=1000&color=00FF41&center=true&vCenter=true&width=650&lines=Marcin+Przybylski;In%C5%BCynieria+i+Analiza+Danych;Data+%2B+ML+%2B+SQL+%2B+Linux;3+rok+%7C+Politechnika+Rzeszowska" alt="Typing SVG" />

![Profile Views](https://komarev.com/ghpvc/?username=marcinprzybylski&label=PROFILE%20VIEWS&color=00FF41&style=flat-square)
![Status](https://img.shields.io/badge/status-online-00FF41?style=flat-square&logo=statuspage&logoColor=black)
![Uptime](https://img.shields.io/badge/uptime-3%20lata-00FF41?style=flat-square)

</div>

<br>

```bash
$ cat about_me.txt
```

> Student III roku **Inżynierii i Analizy Danych** na **Politechnice Rzeszowskiej** (semestr 6).
> Solidne podstawy w analizie danych, zarządzaniu bazami danych i programowaniu.
> Rzetelny. Ciekawy nowych wyzwań. Zawsze `git commit` przed deadlinem. 😉

<br>

## `>_` Aktualny proces

```yaml
proces:
  nazwa: "praca_inzynierska"
  status: "IN_PROGRESS"
  temat: "Optymalne Rozsadzenie Gości Weselnych (Wedding Seating Problem)"
  metoda: "algorytmy kolorowania grafów"
  repo: "wsp"

dodatkowo:
  - kolo_naukowe: "Machine Learning @ PRz"
  - projekt: "AI wspierające diagnostykę weterynaryjną"
  - cel: "przyspieszenie i usprawnienie diagnozy"
```

<br>

## `>_` Zainteresowania i rozwój

<table>
<tr>
<td width="50%" valign="top">

```
[■■■■■■■■□□] Big Data
[■■■■■■■□□□] Business Intelligence
[■■■■■■□□□□] Machine Learning
[■■■■■□□□□□] Sztuczna Inteligencja
```

</td>
<td width="50%" valign="top">

📊 **Big Data** — przetwarzanie i analiza dużych zbiorów danych
📈 **BI** — dane wspierające decyzje biznesowe
🤖 **ML** — modele predykcyjne i systemy uczące się
🧠 **AI** — inteligentne systemy

</td>
</tr>
</table>

<br>

## `>_` Stack technologiczny

<div align="center">

**Języki**

![Python](https://img.shields.io/badge/-Python-000000?style=for-the-badge&logo=python&logoColor=00FF41)
![R](https://img.shields.io/badge/-R-000000?style=for-the-badge&logo=r&logoColor=00FF41)
![SQL](https://img.shields.io/badge/-SQL-000000?style=for-the-badge&logo=postgresql&logoColor=00FF41)

**Bazy danych**

![MSSQL](https://img.shields.io/badge/-Microsoft%20SQL%20Server-000000?style=for-the-badge&logo=microsoftsqlserver&logoColor=00FF41)
![MySQL](https://img.shields.io/badge/-MySQL-000000?style=for-the-badge&logo=mysql&logoColor=00FF41)
![Oracle APEX](https://img.shields.io/badge/-Oracle%20APEX-000000?style=for-the-badge&logo=oracle&logoColor=00FF41)

**Narzędzia i środowisko**

![Linux](https://img.shields.io/badge/-Linux-000000?style=for-the-badge&logo=linux&logoColor=00FF41)
![Excel](https://img.shields.io/badge/-Excel-000000?style=for-the-badge&logo=microsoftexcel&logoColor=00FF41)
![PowerPoint](https://img.shields.io/badge/-PowerPoint-000000?style=for-the-badge&logo=microsoftpowerpoint&logoColor=00FF41)
![LaTeX](https://img.shields.io/badge/-LaTeX-000000?style=for-the-badge&logo=latex&logoColor=00FF41)

</div>

<br>

## `>_` GitHub stats

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=marcinprzybylski&show_icons=true&theme=chartreuse-dark&hide_border=true&bg_color=0D1117&title_color=00FF41&icon_color=00FF41&text_color=c9d1d9" width="48%" />
<img src="https://github-readme-streak-stats.herokuapp.com/?user=marcinprzybylski&theme=github-dark-blue&hide_border=true&background=0D1117&ring=00FF41&fire=00FF41&currStreakLabel=00FF41" width="48%" />

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=marcinprzybylski&layout=compact&theme=chartreuse-dark&hide_border=true&bg_color=0D1117&title_color=00FF41&text_color=c9d1d9" width="48%" />

</div>

> ⚠️ **Uwaga:** podmień `marcinprzybylski` na swój prawdziwy login GitHub, żeby statystyki się wyświetliły.

<br>

## `>_` Nawiąż połączenie

```bash
$ ping marcin_przybylski --contact
```

<div align="center">

[![Email](https://img.shields.io/badge/-mar.przybylski@o2.pl-000000?style=for-the-badge&logo=gmail&logoColor=00FF41)](mailto:mar.przybylski@o2.pl)
[![Business Email](https://img.shields.io/badge/-marcin.przybylski@toyota--ce.com-000000?style=for-the-badge&logo=maildotru&logoColor=00FF41)](mailto:marcin.przybylski@toyota-ce.com)

</div>

<br>

<div align="center">

```
$ echo "Dziękuję za odwiedzenie mojego profilu!"
> Dziękuję za odwiedzenie mojego profilu!

$ exit
[process finished]
```

![Footer](https://capsule-render.vercel.app/api?type=waving&color=0:0D1117,100:00FF41&height=120&section=footer)

</div>
